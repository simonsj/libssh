...
static void torture_dhgroup_better_size(void **state) {
    ...
    for (i=0; i<3;++i){
        best = 0;
        for (j=0; j< 5; ++j){
            if(dhgroup_better_size(512,1024,2048,best, groups[i][j][0])){
                best = groups[i][j][0];
...
static void torture_retrieve_dhgroup_file(void **state){
    FILE *moduli = tmpfile();
    ...
    rewind(moduli);
    ssh_retrieve_dhgroup_file(moduli, 1024, 2048, 4096,
        &size, &generator, &modulus);
