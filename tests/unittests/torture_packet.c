...
static SSH_PACKET_CALLBACK(copy_packet_data){
    ...
    size_t len = buffer_get_rest_len(packet);
    (void)type;
    (void)session;
    ...
    buffer_get_data(packet, response, len);
...
int torture_run_tests(void) {
    int rc;
    UnitTest tests[] = {
        unit_test(torture_packet_aes128_ctr),
        unit_test(torture_packet_aes192_ctr),
        unit_test(torture_packet_aes256_ctr),
        unit_test(torture_packet_aes128_cbc),
        unit_test(torture_packet_aes192_cbc),
        unit_test(torture_packet_aes256_cbc),
        unit_test(torture_packet_3des_cbc),
        unit_test(torture_packet_chacha20)
    };
