...
#ifdef DEBUG_CRYPTO
    fprintf(stderr, "%d bits, %d bytes, %d padding, %d min_size\n",
                    bits, len, pad, min_size);
#endif /* DEBUG_CRYPTO */
...
static ssh_string dh_get_k(ssh_session session) {
    ...
    case SSH_KEX_CURVE25519_SHA256_LIBSSH_ORG:
        /*
         * Workaround for an OpenSSH bug:
         * -> https://red.libssh.org/issues/159
         * -> https://bugzilla.mindrot.org/show_bug.cgi?id=2233
         */
        openssh_version = ssh_get_openssh_version(session);
        if ((openssh_version == SSH_VERSION_INT(6, 5, 0)) ||
            (openssh_version == SSH_VERSION_INT(6, 6, 0))) {
